...
public class EvaluateurNiveau {
	...
	public static String evaluerNiveau(String cours, String examen)
		...
		if (!estEntier(examen)) {
			throw new ExceptionNoteNonEntiere("Examend doit être un entier");
		}
		...
		if (!estDansInterval(icours, MIN_COURS, MAX_COURS)) {
			throw new ExceptionNoteHorsBorne("La note doit être dans la borne");
		}
		if (!estDansInterval(iexamen, MIN_EXAMEN, MAX_EXAMEN)) {
			throw new ExceptionNoteHorsBorne("Examen pas dans la borne");
		}
